...
class PretrainConfig:
    ...
    lm: int = 3
    # Multi-window: `current_views` = W currents drawn per anchor, EACH sampled
    # exactly as official TimeSiam (random within [s, s+sampling_range*L]). The
    # ONLY change vs TimeSiam is W; W=1 is bit-exact to TimeSiam. `w_chunk_size`
    # > 0 chunks the W axis to bound VRAM (single optimizer.step per batch; loss
    # globally re-weighted so the gradient matches the single-pass).
    current_views: int = 1
    w_chunk_size: int = 0
    device: str = "auto"
    seed: int = 2021
...
class PretrainModel(nn.Module):
    ...
    def forward(
        ...
        """Faithful multi-window pass: each of the W currents is reconstructed
        against `past` re-encoded with that window's lineage token (W past
        encodings — the lineage token is added before the encoder, so past is
        NOT amortized). W is folded into the batch dim. Returns un-normalized
        accumulators `sq_sum` / `denom` so an outer chunked loop can divide by a
        global denominator (exact equivalence to the single-pass mean)."""
        ...
        if segments.shape != (batch_size, num_views):
            raise ValueError(
                f"Expected segments shape [B, W={num_views}], got {tuple(segments.shape)}"
            )

        # Flatten W into the batch dim: each (b, w) is its own window.
        currents_flat = currents.reshape(batch_size * num_views, input_len, num_features)
        # mask is built per (b, w) by the caller: shape [B*W, L, C].
        if mask.shape != (batch_size * num_views, input_len, num_features):
            raise ValueError(
                f"Expected mask shape [B*W={batch_size * num_views}, {input_len}, "
                f"{num_features}], got {tuple(mask.shape)}"
            )

        if self.config.use_norm:
            past_norm, _, _ = masked_normalize(past)
            cur_norm, cur_means, cur_stdev = masked_normalize(currents_flat, mask=mask)
        else:
            past_norm = past
            cur_norm = currents_flat
            cur_means = torch.zeros_like(currents_flat[:, :1, :])
            cur_stdev = torch.ones_like(currents_flat[:, :1, :])

        # Past, encoded W times (faithful: lineage token depends on the window).
        past_w = (
            past_norm.unsqueeze(1)
            .expand(batch_size, num_views, input_len, num_features)
            .reshape(batch_size * num_views, input_len, num_features)
        )
        past_emb, _, n_vars = self.patch_embedding(past_w, add_pos=True)
        seg_flat = segments.reshape(batch_size * num_views)
        past_emb = past_emb + self._select_past_token(seg_flat, n_vars, past_emb.size(1))
        ...
        pred = patch_pred.reshape(
            batch_size * num_views, num_features, num_patches * self.config.patch_len
        )
        ...
        squared_error = (pred - currents_flat) ** 2
        sq_sum = (squared_error * loss_mask).sum()
        denom = loss_mask.sum()
        return {"sq_sum": sq_sum, "denom": denom, "pred": pred}
...
def _run_epoch(
    ...
    steps = 0
    cfg = model.config
    chunk = int(getattr(cfg, "w_chunk_size", 0) or 0)
    context = torch.enable_grad() if is_train else torch.no_grad()
    with context:
        for batch_idx, (past, currents, segments) in enumerate(loader):
            if max_steps is not None and batch_idx >= max_steps:
                break
            if is_train:
                optimizer.zero_grad(set_to_none=True)
            bsz = past.size(0)
            num_views = currents.shape[1] if currents.dim() == 4 else 1
            # One mask per (b, w); flattened [B*W, L, C] aligns with currents'
            # B-outer / W-inner flattening inside forward().
            mask = build_mask(
                cfg.masked_rule,
                bsz * num_views,
                cfg.input_len,
            ...
            total_denom = (~mask).float().sum().clamp_min(1.0)

            if chunk > 0 and num_views > chunk:
                # Chunk the W axis: accumulate gradients, single optimizer.step.
                # Each chunk's loss is divided by the GLOBAL denom, so the summed
                # chunk losses equal sq_sum_total / total_denom (= single-pass).
                mask_bw = mask.view(bsz, num_views, cfg.input_len, cfg.enc_in)
                batch_sq = 0.0
                for c0 in range(0, num_views, chunk):
                    c1 = min(c0 + chunk, num_views)
                    cur_c = currents[:, c0:c1]
                    seg_c = segments[:, c0:c1]
                    mask_c = mask_bw[:, c0:c1].reshape(
                        bsz * (c1 - c0), cfg.input_len, cfg.enc_in
                    )
                    out_c = model(past, cur_c, seg_c, mask_c)
                    loss_c = out_c["sq_sum"] / total_denom
                    if is_train:
                        loss_c.backward()
                    batch_sq += float(out_c["sq_sum"].detach().cpu())
                batch_loss = batch_sq / float(total_denom.detach().cpu())
            else:
                output = model(past, currents, segments, mask)
                loss = output["sq_sum"] / total_denom
                if is_train:
                    loss.backward()
...
def _add_config_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--sampling_range", type=int, required=True,
                        help="Sample each current within [0, sampling_range*input_len] past the "
                             "anchor (TimeSiam-identical; continuous timesteps).")
    ...
    parser.add_argument("--current_views", type=int, default=1,
                        help="W: number of currents drawn per anchor. W=1 = bit-exact TimeSiam.")
    parser.add_argument("--w_chunk_size", type=int, default=0,
                        help="Chunk the W axis to bound VRAM (0 = single pass). "
                             "Single optimizer.step/batch; gradient is exact.")
...
def _config_from_args(args: argparse.Namespace) -> PretrainConfig:
    return PretrainConfig(
        ...
        w_chunk_size=args.w_chunk_size,
