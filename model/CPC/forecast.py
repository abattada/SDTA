...
class CPCForecaster(nn.Module):
    """CPC finetune model with repo shared patch encoder and flatten head.

    Identical in shape to the other shared-encoder forecasters (SimMTM / TimeSiam):
    the CPC pretext only changes how `patch_embedding` + `encoder` are *pretrained*
    (autoregressive InfoNCE), not the downstream forecasting path. The encoder runs
    NON-causal at forecast time (it was trained causally as the CPC `g_ar`; same
    pretrain-causal / forecast-noncausal pattern as TimeDART), so this path stays
    byte-for-byte apples-to-apples with the other baselines.
    """
